...
この違いが、「なぜ Groth16 と STARK が共存しているのか」「なぜ PLONK に移行するチームが多いのか」を決める。本記事では、応用ごとに何が重視されるかと SNARK の性質との**対応を本記事だけで俯瞰できる**ように整理する。
...
Ethereum L1 は**秒あたり約15〜30件**（**2026年5月現在**、L1に記録される取引**件数**から見た実効スループットの目安。混雑やプロトコル改修で変動）のトランザクションしか処理できない。Visa の1万件/秒とは桁違いに遅い。セキュリティと分散性を保つためにこれは意図的だが、ユーザー増加に追いつかない。
...
| **Knowledge** | ★★☆ | 正当な遷移を回路が検算する **witness**（実行トレース・Merkle 証明など）を、証明者が**知っている**ことの保証。|
| **Zero-Knowledge** | ★☆☆ | 通常は ZK 性なし（プライバシー不要） |

多くの zkRollup は **ZK 性なしの SNARK** を使う。ステートは公開情報なので、プライバシーは目的ではない。**Knowledge は「witness を隠す」ではなく「捏造できない」ことに効く**。
...
**L2 に載せないから秘密、ではない。** シールド取引は **Zcash 自身の L1** 上で検証される。プライバシーの本質は「オフチェーンに隠す」ことではなく、**オンチェーンに載せる情報を、追跡しにくい形に限定する**ことだ。Ethereum の zkRollup が通常プライバシーにならないのは、データ可用性のために **トランザクション内容が第三者からも再構成できる形で L1 に載る** ことが多いからである。一方シールドでは、プレーンテキストの送金履歴の代わりに、次に述べる **コミットメント・Nullifier・証明** だけで整合性を示す。

**コミットメント**は、「裏にある秘密（金額や受取に関する情報など）」をこの場では明かさないまま、**後からねじ曲げられない約束**としてチェーンに残す値だ。イメージとしては、内容は伏せたまま「この封筒があなたの新しい受取権に対応する」という**公開ラベルだけ**を貼るようなもので、Zcash では新しい **ノート**（受け取り権の単位）がコミットメントとして記録される。本人は秘密情報を持っているので、後から ZK 証明と組み合わせて「このコミットメントに対応する権利を正当に消費した」と主張できる。

**Nullifier**は、ノートを**一度支出したこと**を全員に示すための、**支出のたびに一度だけ公開されるタグ**だ。チェーンは「この Nullifier は過去に使われたか」を見るだけで二重支払いを防げる。重要なのは、**Nullifier だけを見ても、どの入金（どのコミットメント）から生まれた支出かが分からない**ように設計されている点で、送金のグラフ上のリンクを切る役割を持つ（厳密な定義はプロトコル世代ごとに異なるが、直感は共通している）。
...
- **コミットメント**（新しいノート＝受取枠の約束）
- **Nullifier**（使い切ったノートの無効化タグ）
- **証明 $\pi$**
...
flowchart TD
    Start[アプリを決める]
    Q1{プライバシー必要?}
    Q2{PQ耐性必要?<br/>量子攻撃後も検証を信頼したいか}
    Q3{Setup許容?<br/>Trusted setup / SRS を許すか}
    Q4{Prover時間重視?}
...
**PQ耐性必要?** … **将来の量子計算機**を想定し、証明の検証がそのときも暗号学的に妥当だとみなしたいか、という問いだ。楕円曲線の離散対数やペアリングに依存する系（典型的な **Groth16 / KZG 系 PLONK** など）は、長期的には量子攻撃の対象になりうる。**ハッシュ関数の仮定**を主に使う **STARK / FRI** 系は PQ 耐性を売りにしやすい。ブリッジや数十年単位で検証される真正性など**ライフサイクルが長い**用途では Yes に寄りやすい。逆に、数年以内に鍵や経済的価値がリセットされる前提だけなら No も現実的で、その場合は証明サイズやエコシステムの都合で楕円曲線系が選ばれることが多い。

**Setup許容?** … 多くの楕円曲線ベースの SNARK（**Groth16** や **KZG を使う PLONK** など）は、証明・検証の前に **公開パラメータ**が要る。そのパラメータは本来「秘密の乱数」から作るが、**その乱数そのものは絶対に残してはいけない**（残ると偽の証明が作れることがある）。この **秘密乱数の扱いとパラメータ生成プロセス**をまとめて **Trusted setup** と呼ぶことが多い。

**SRS（Structured Reference String）** は、セットアップの結果として世界中に配布される **構造化参照文字列**＝Prover/Verifier が実際に使う**公開パラメータの束**だ。名前のとおり「文字列」だが、中身は群の元などが規則的に並んだデータで、**回路の制約に合わせて**（または **Universal** なら「ある上限サイズまでの回路なら共通」で）用意する。**回路ごと**（典型: Groth16）はその回路専用の SRS を新たに生成する必要があり、**Universal**（典型: PLONK 系）は一度のセレモニーで幅広い回路に再利用できるが、いずれも「生成時に秘密が漏れなかったか」を運用でどう扱うかが論点になる。

実務では **MPC セレモニー**（複数参加者が断片を出し合い、**全員が不正でもうまくいかない**ようにする等）で信頼を薄める。**Setup を許容する**とは、「そのプロセスと、将来の **SRS アップグレード**（より大きな回路へ拡張するときの再セットアップなど）を、自プロダクトの脅威モデルで受け入れられるか」という意味だ。許容できるなら **KZG** 系が選びやすく**証明は小さくなりやすい**。**誰も trapdoor を持たない**という **Transparent**（ハッシュ中心の **STARK** など）を絶対条件にするなら、図のとおり STARK 寄りになる。zkEVM や **zkML** でこの軸が議論に出るのは、**信頼の置き場所を減らしたい**からだ。
...
次の記事では、SNARK / STARK / Bulletproofs / Halo2 など主要プロトコルを **分類マップ**で整理する。どこで分岐しているのか、なぜこれだけ種類があるのか、どう比較すればよいのか。技術選択の基盤となる理解を固める。
