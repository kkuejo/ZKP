...
ここで $\Pr_{r_V}[\cdot]$ という記法は、「**Verifierの内部ランダムネス $r_V$（コイン投げの結果）を一様ランダムに選んだときの、事象 $[\cdot]$ が起こる確率**」を意味する。下付きの $r_V$ は「どの確率変数についての確率なのか」を明示するためのもので、Proverと Verifier の入力 $x$ や Witness $w$ は固定したまま、$r_V$ だけを動かしたときの確率を取る、ということを表している。

つまり:

- Proverが本物のWitness $w$ を持っている
- Verifierのランダムコイン $r_V$ について確率を取る（$x, w$ は固定）
- その確率で受理される
...
健全性のところで「悪意あるProver $P^\ast$ がどんなズルい戦略を使っても騙せない」と書いたが、実はこの「**どんな** $P^\ast$ を相手にするか」によって、健全性には**強さの違う2種類**がある。

#### イメージで言うと

「金庫が安全」と一言で言っても:

- **A: バールでもバーナーでも、何時間かけても開かない金庫**
- **B: 普通の泥棒が現実的な時間内（一晩とか）で開けることはできない金庫**

A の方が当然強い。でも実際の銀行が使っている金庫はだいたい B のタイプで、「現実的な道具・時間では破れない」ことを保証している。ZKPの世界にも、これとよく似た2段階の安全性がある。

| 名前 | 想定する悪意あるProver $P^\ast$ | 強さ |
|---|---|---|
| **Statistical Soundness（統計的健全性）** | 計算能力に**制限なし**（時間も計算資源も無限） | 強い（金庫A） |
| **Computational Soundness（計算量的健全性）** | $P^\ast$ は**多項式時間**しか動けない（＝現実的な計算機で動く範囲） | 弱い（金庫B） |

#### Proof と Argument の呼び分け

- **Proof（証明）**：統計的健全性を満たすシステム。どんな計算機を使っても、たとえ宇宙が終わるまで計算しても騙せない。
- **Argument（議論）**：計算量的健全性しか満たさないシステム。「現実的な計算量で動く攻撃者」には騙されない、という保証。

「Argument」という日本語訳は「議論」だが、ここでは「**現実世界の計算能力という前提のもとでの説得**」というニュアンス。Proofは数学的に絶対、Argumentは「実用上は破られない」というレベル感の違いだと思えばよい。

#### 現代のSNARKはどっち？

実は、みんなが使っている多くのSNARKは「**Succinct Non-interactive Argument of Knowledge**」の略で、名前のとおり **Argument** の方である（Proofではない）。

つまり厳密に言えば:

- 「無限の計算能力を持つ攻撃者」がいたら、理論的には騙せてしまう可能性がある
- でも、そんな攻撃者は**現実には存在しない**

ので実用上は安全に使える、ということ。RSA暗号などの公開鍵暗号も「素因数分解が現実的な時間で解けない」という前提で安全とされているのと同じ発想だ。

なお「**量子計算機でない限り安全**」と書いたのは、SNARKの安全性が依拠している数学的な難問（離散対数問題など）の一部は、十分強力な量子計算機があれば解けてしまうことが知られているため。だから将来の量子時代に備えて、量子計算機にも耐える ZKP（STARKなど）の研究が進んでいる、という話につながっていく。
...
ゼロ知識性を理解するうえで一番のハードルは、「**情報が漏れていない、ってそもそもどう数学的に書くの？**」という点にある。「秘密 $w$ がViewに含まれていない」と直接書こうとしても、$w$ は暗号化された形や統計的にぼやけた形で混ざっているかもしれず、「含まれていない」を厳密に定義するのは難しい。

ここで Goldwasser-Micali-Rackoff（GMR）の3人が天才的なアイデアを出した。発想を逆転させて、こう考えたのだ:

> **「Verifierが対話で見たものは、実は秘密 $w$ を知らなくても自分で作れる」と示せれば、それは情報が漏れていない証拠だ**

#### たとえ話：手品の種明かし

たとえば、あなたが手品師のショーを見て、ショーの全記録（映像）を持ち帰ったとしよう。後日、その映像を友人に見せて「すごいだろ？」と自慢する。

ところが友人がこう言ったとする:

> 「いや、その映像、**手品の種を一切知らない俺でも、自宅のPCで作れるよ**。ほら、CGで全く同じ映像を作ってみた。見分けつかないでしょ？」

このとき、あなたが持ち帰った映像には**手品の種に関する情報は含まれていない**ことになる。なぜなら、種を知らない人でも同じ映像を作れたのだから、その映像から種を学べるはずがない。

ZKPはこれと同じ理屈だ:

- 手品師 = Prover、観客 = Verifier、手品の種 = Witness $w$
- ショーの記録映像 = View（Verifierが対話で見たすべて）
- 友人のCG職人 = **シミュレータ $\text{Sim}$**

#### 形式的なアイデア

これを **シミュレーションパラダイム** といい、次のように書ける:

> **$x$（主張）だけを入力として、本物のViewと区別できない偽Viewを作れる多項式時間アルゴリズム $\text{Sim}$（シミュレータ）が存在する**

ポイントは:

- **$\text{Sim}$ は Witness $w$ を一切使わない**（$x$ だけで動く）
- それなのに、本物の対話ログとそっくりな偽ログを作れる
- → だったら本物のログにも $w$ の情報は実質的に含まれていない

「**$w$ を知らないやつでも作れるもの**から、$w$ の情報を抽出できるわけがない」という、ある意味当たり前の論理を、暗号理論の言葉に乗せた定義だ。これがゼロ知識性の核心アイデアになっている。
