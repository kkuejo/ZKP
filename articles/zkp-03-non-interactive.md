...
**問題2: 検証者が複数いると「コミットの使い回し」で秘密が抜きうる**

対話型 ZKP は「1対1の対話」が基本だ。Verifier が2人いて**同じコミットメント（同じ一時乱数）**に対し、それぞれ別のチャレンジを出したとする。Prover がどちらにも正直に応答すれば応答値はチャレンジごとに異なるが、**2本のトランスクリプトを並べると、証明者の秘密（例: Schnorr 型では離散対数の秘密鍵）が代数的に復元できる**（nonce の再利用問題）。「応答同士が論理的に矛盾する」というより、**同じ乱数に対する2つの線形な制約が、秘密を一意に決めてしまう**イメージである。
...
**誰が $\sigma$ を作るのか** が問題になる。

- **Prover が $\sigma$ を自由に作れる**場合、健全性（Soundness）が危うい。偽の $\sigma$ を仕込めば、本来は偽であるはずの証明を Verifier に受理させられる可能性がある。
- **Verifier だけが $\sigma$ を自由に作れる**場合も、ゼロ知識性（ZK）の議論の前提が崩れうる。多くの NIZK の ZK 証明は「$\sigma$ は正直な `Setup` が出力する分布に従う」と仮定し、そのもとで**シミュレータ**が witness を知らずに $(x,\pi)$ を再現できることを示す。Verifier が $\sigma$ を戦略的に選べると、実質**公開パラメータを検証側が設計できる**ことになり、$\sigma$ が偏ったりトラップドアを含んだりすると、想定したシミュレーションが成立しなくなったり、証明から witness に関する余分な情報が漏れたりする（敵対的 CRS）。

だから実システムでは、下表のように **$\sigma$ の生成を信頼できる過程に縛る**（TTP・MPC・透明セットアップなど）必要がある。

**解決策の種類**:

| CRSの生成方法 | 特徴 | 例 |
|---|---|---|
| **信頼できる第三者（TTP）** | セレモニーで1人が生成し、公開 CRS を確定したあと**秘密（toxic waste / trapdoor）を破棄**する。保持し続けると設計次第で**偽証**が可能になり、信頼モデルが崩れる。 | 昔のBLS署名 |
...
ここで $\{0,1\}^\ast$ の添字 $*$（アスタリスク）は **Kleene 星**で、「0 と 1 からなる**有限列の全体**」、すなわち**長さ 0 以上の任意長**のビット列（空列も含む）を表す。$\{0,1\}^\lambda$ は長さちょうど $\lambda$ のビット列の集合であり、$\lambda$ は抽象的な記号だが、**実装では採用するハッシュ関数の出力長で決まる**（例: SHA3-256 なら $\lambda = 256$）。
...
上表で CRS 側にあった **Trusted Setup（トラステッドセットアップ）**＝秘密付きセレモニーへの依存と対になる言葉として、**Transparent Setup** が使われる。**Transparent Setup** とは、**そのような信頼できる第三者・秘密破棄の儀式を要しない**セットアップの総称である。ROM ベースの構成（FRI-based STARKs など）が代表例。
...
- **zkRollup**: L2 が大量のトランザクションをまとめ、**その集約結果が正しい**ことを L1 に載せる小さな証明で示すスケーリング手法（Ethereum などで典型）。
- **プライバシー決済**: 送金額や送受者などを公開チェーンに載せず、**トランザクションの正当性だけ**を ZK で示す（匿名コイン・ミキサ等）。
- **ZK-ID**: 氏名・生体・クレデンシャル本体を開示せず、**属性や年齢閾値などの条件を満たす**ことだけを証明するアイデンティティ／認証。
- **zkML**: モデルや入力を丸ごと公開しないまま、**推論や学習のあるステップが約束どおり行われた**ことを証明する（モデル提供者・利用者の両方の秘匿に使われる）。
