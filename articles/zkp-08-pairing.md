...
**$\text{div}(f)$ とは**: 関数 $f$ が、曲線上の**どの点で 0 になるか**（零点）、**どの点で発散するか**（極）を記録したもの。代数幾何の記法。

右辺を素朴に読む:

| 記号 | 意味 |
|---|---|
| $n(P)$ | 点 $P$ で **$n$ 重の零点**（$f$ が $P$ で $n$ 回 0 になる） |
| $-(nP)$ | 点 $nP$ で **1 重の零点** |
| $-(n-1)(\mathcal{O})$ | 無限遠点 $\mathcal{O}$ で **$n-1$ 重の極**（発散） |

**何のための式か**: $P$ を $n$ 倍する double-and-add で使う直線・接線を全部掛け合わせると、ちょうどこの零点・極のパターンを持つ関数 $f_{n,P}$ ができる、ということを言っている。

**具体例** ($n = 5$): $5P$ を求めるとき、接線・直線を5〜6本 $f$ に掛け込む。作られた $f_{5,P}$ は:

- 点 $P$ では 5 回 0 になる（$5(P)$ に対応）
- 点 $5P$ では 1 回 0 になる
- 無限遠 $\mathcal{O}$ では $4$ 回発散する

この「零点と極の釣り合い」があるから、$f_{n,P}(Q)$ を別の点 $Q$ で評価した値が、ペアリング計算に使える正しい情報になる。

**まとめ**: 実装・検証の理解には「$nP$ の経路の直線・接線を $f$ に掛け込む」で十分。この $\text{div}$ の式は、その裏付けを代数幾何で書いたもの。

#### 簡略例: $n = 5$ のとき Miller ループのイメージ

$5P$ を **double-and-add**（2進展開）で求める。$n = 5 = 101_2$（MSB から処理）。

| ステップ | 操作 | $T$ の状態 | $f$ に掛けるもの |
|---|---|---|---|
| 初期 | — | $T = \mathcal{O}$ | $f = 1$ |
| bit 2 (=1) | 最初の 1 | $T = P$ | — |
| bit 1 (=0) | 2倍 | $T = 2P$ | $P$ における**接線** |
| bit 0 (=1) | 2倍 | $T = 4P$ | $2P$ における**接線** |
| | 加算 | $T = 4P + P = 5P$ | $4P$ と $P$ を結ぶ**直線** |

最後の加算は $4P + P = 5P$。**$12P + P = 13P$ ではない**。

以前の表では、Miller 疑似コードの「初期 $T = P$」という取り方で追跡していたため、最後に $12P + P = 13P$ という中間値が出ていた。しかし $5P$ を求める正しい double-and-add では、上表のように $4P + P = 5P$ で終わる。

Article 7 の $\mathbb{F}_5$ 上の曲線で $P = (1, 1)$ なら、加算公式で $5P = (1, 4)$（位数 7 の群で $5P = -P$）。
