...
Miller loop 全体では、$11P$ を求める 3 本の接線と 2 本の直線それぞれについてこの操作を繰り返し、最終的な $f$ を得る。Article 7 では直線・接線は「点を求めるため」に使って捨てていたが、Miller では**捨てずに $f$ に積み上げる**。
...
**具体例** ($n = 11$): $11P$ を求めるとき、接線・直線を $f$ に掛け合わせる（次の表では接線 3 本と直線 2 本。$11 = 1011_2$）。作られた $f_{11,P}$ の零点・極は次のとおり。

- **点 $P$ で 11 重零点**（$11(P)$ に対応）

  $P$ における**接線**は **2 重零点**を持つ。2 倍のたびに $f \leftarrow f^2 \times (\text{接線})$ とするので、$P$ での重数は 1 段階で 2 倍される。加算の**直線**は $P$ を通るので **1 重零点**を 1 本あたり追加する。$11 = 1011_2$ の経路では:

  1. bit 2 (=0) の 2 倍: $f = \ell_P$ → $P$ で **2 重**
  2. bit 1 (=1) の 2 倍: $f = \ell_P^2 \cdot \ell_{2P}$ → $P$ で **4 重**
  3. bit 1 (=1) の加算: $4P$ と $P$ を結ぶ直線を掛ける → $P$ で **+1 重** → **5 重**
  4. bit 0 (=1) の 2 倍: $f \leftarrow f^2 \times \ell_{5P}$ → $P$ で **10 重**
  5. bit 0 (=1) の加算: $10P$ と $P$ を結ぶ直線を掛ける → $P$ で **+1 重** → 合計 **11 重**

  $2 \to 4 \to 5 \to 10 \to 11$ と進む。2 倍で倍々、加算で $+1$ ずつ増える様子がはっきり見える。

- **点 $11P$ で 1 重零点**

  最終加算 $10P + P = 11P$ のとき、$10P$ と $P$ を結ぶ**直線**は $10P$ でも $P$ でも 0 になる。スカラー倍の**結果点** $11P$ に 1 重零点が残る。$\text{div}$ 式の $(11P)$ 項がこれに対応する。

- **無限遠 $\mathcal{O}$ で 10 重極**（$n - 1 = 10$）

  楕円曲線上の直線 $\ell(x, y) = ax + by + c$ は、プロジェクティブ座標では $\mathcal{O}$（無限遠）で**極**（発散）を持つ。Miller loop で掛けた関数全体では、$\mathcal{O}$ に **$n - 1$ 重**の極が残る。$P$ で 11 重・$11P$ で 1 重の零点と、$\mathcal{O}$ で 10 重の極は**除子として次数 0**（零点と極が釣り合う）になり、有理関数として well-defined になる。
...
#### 簡略例: $n = 11$ のとき Miller ループのイメージ

$11P$ を **double-and-add**（2進展開）で求める。$n = 11 = 1011_2$（MSB から処理）。表の **2進桁** 列は、Miller loop が $n$ の2進表現を**上の桁（bit 3）から下の桁（bit 0）へ**1桁ずつ読む順番を表す。

| 2進桁 | 操作 | $T$ の状態 | $f$ に掛けるもの |
|---|---|---|---|
| 初期 | — | $T = \mathcal{O}$ | $f = 1$ |
| bit 3 (=1) | 最初の 1 | $T = P$ | — |
| bit 2 (=0) | 2倍 | $T = 2P$ | $P$ における**接線** $\ell_P$ |
| bit 1 (=1) | 2倍 | $T = 4P$ | $2P$ における**接線** $\ell_{2P}$ |
| bit 1 (=1) | 加算 | $T = 4P + P = 5P$ | $4P$ と $P$ を結ぶ**直線** |
| bit 0 (=1) | 2倍 | $T = 10P$ | $5P$ における**接線** $\ell_{5P}$ |
| bit 0 (=1) | 加算 | $T = 10P + P = 11P$ | $10P$ と $P$ を結ぶ**直線** |

$1011_2$ では **2倍 3 回**と**加算 2 回**が起きる。$T$ は $P \to 2P \to 4P \to 5P \to 10P \to 11P$ と進む。bit が 0 なら 2倍だけ、bit が 1 なら 2倍してから $P$ を足す（最初の 1 だけは $T = P$ から開始）。各 2倍・加算のたびに、対応する接線または直線を $f$ に掛ける。下の疑似コードは、この表と同じ手順を書いたもの。

**なぜ $8P$ を経由しないか**: bit 1 (=1) では $2P \to 4P$ と 2 倍した**直後に** $P$ を足す。$4P \to 8P$ の 2 倍はこのタイミングでは行わない（$8P$ が現れるのは $n = 8 = 1000_2$ のように途中に加算がない場合）。次の 2 倍は $5P \to 10P$ なので、$T = 8P$ という状態は一度も起きない。$4P \to 8P \to 10P$ という並びも、2 倍算の規則に合わない（$8P$ の 2 倍は $16P$ であり $10P$ ではない）。
...
| 行 | 何をしているか | なぜ |
|---|---|---|
| `T = None` → 最初の 1 で `T = P` | MSB から読み、先頭の 0 はスキップ | $1011_2$ なら最初に $T = P$、その後 2倍×3・加算×2 で $11P$ |
...
BLS では署名者が秘密鍵 $x$ を使い、$\sigma = xH$ と計算する（$\mathbb{G}_1$ 上の**スカラー倍**）。本物の実装では $H = \text{HashToCurve}(m)$ だが、ここでは計算を簡単にするため $H = g^3$（$= 2^3 = 8$）と置く。

1. **$\sigma = xH$ を指数で書く**

   簡略モデルでは点 $g^a$ は指数 $a$ で表せる。$H = g^3$ なので、$x = 5$ 倍すると:

   $$
   \sigma = 5H = 5 \cdot g^3 = g^{5 \times 3} = g^{15}
   $$

   「秘密鍵 5 を $H$ に掛ける」=「指数 3 に 5 を掛ける」という対応。

2. **指数は位数 12 で割った余り**

   この群では $g^{12} \equiv 1 \pmod{13}$（1.5 節で確認済み）なので、指数は $\bmod 12$ で同じ点を表す:

   $$
   15 = 12 + 3 \quad \Rightarrow \quad g^{15} = g^{12} \cdot g^3 = 1 \cdot g^3 = g^3
   $$

3. **数値に戻す**

   $$
   \sigma = g^3 = 2^3 = 8 \pmod{13}
   $$

   署名 $\sigma$ は公開情報 $H = g^3$ からは $x = 5$ が分からない（ECDLP）。検証者は次のペアリング等式だけを確かめる。

**検証** $e(\sigma, G_2) \stackrel{?}{=} e(H, X)$:

左辺: $e(g^3, g) = g^3 = 2^3 \equiv 8 \pmod{13}$

右辺: $e(g^3, g^5) = g^{15} \equiv 2^{15} \equiv 2^{12} \cdot 2^3 \equiv 1 \cdot 8 = 8 \pmod{13}$
...
$\sigma_{\text{agg}} = g^3 \cdot g^9 = g^{12} \equiv 1 \pmod{13}$
...
右辺: $e(g^2, g^3) \cdot e(g^4, g^5) \cdot e(g^3, g^3) = g^6 \cdot g^{20} \cdot g^9 = g^{35}$
