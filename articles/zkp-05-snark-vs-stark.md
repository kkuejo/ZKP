---
title: "[ゼロ知識証明入門シリーズ 5/33] ZKPの分類 — SNARK vs STARK、そして分類マップを描く"
emoji: "🗺️"
type: "tech"
topics:
  - zkp
...
- **Non-interactive** は Fiat-Shamir 変換（Article 18）で実現
...
以上をまとめた **ZKP の分類マップ**（歴史の流れと PCS 系統を1枚にした概略）:

```mermaid
flowchart TB
    ZKP85["対話型 ZKP<br/>1985"]
    NIZK["NIZK<br/>1988"]
    ...
    SNARK --> HashBased["Hash-based<br/>FRI, Merkle"]
    ...
    HashBased --> STARK["STARK 2018<br/>FRI ベース"]
    HashBased --> Plonky2["Plonky2 2022<br/>PLONK + FRI"]
    HashBased --> Brakedown["Brakedown 2021<br/>線形時間 Prover"]
    HashBased --> Binius["Binius 2024<br/>GF(2) 高速"]
```
...
**再帰的な証明生成を効率化する技術**（Article 20）。複数の R1CS インスタンスを1つに「畳み込む」ことで、SNARK 生成コストを分散する。これ自体は SNARK ではなく、SNARK の中に埋め込む「高速化レイヤー」。
...
6. 分類マップに沿った主な流れ: **Groth16 → PLONK → Halo2 → Nova → Binius**
...
- **Poly-IOP × PCS の直交マトリクス** が ZKP の分類マップを形作る
